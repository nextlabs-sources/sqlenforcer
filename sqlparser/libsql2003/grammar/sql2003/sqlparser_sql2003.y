...
%token <node> PARAM
%token <node> CREATE_TABLE
%token <node> CREATE_TABLE_REST
...
%token CALL CASE CAST CHAR CHARACTER CHARACTERS CLOB
       ...
       CONVERT CORRESPONDING COUNT CREATE CROSS CUME_DIST
       CURRENT CURRENT_TIMESTAMP CURRENT_USER
%token DATE DAY DEC DECIMAL DEFAULT DELETE
       DENSE_RANK DESC DISTINCT DOUBLE DROP
%token ELSE END END_P ESCAPE ERROR EXCEPT EXCLUDE EXEC EXECUTE
%token EXISTS
%token FILE_TABLE FLOAT FOLLOWING FOR FROM FULL
%token G GROUP GROUPING
%token HAVING HOUR
%token IF IN INNER INSERT INT INTEGER INTERSECT INTERVAL
       INTO IS
%token JOIN
%token K
...
%token SCOPE SECOND SELECT SESSION_USER SET SETS SMALLINT
       SOME SP_EXECUTESQL STDDEV_POP STDDEV_SAMP SUM SYSTEM_USER
%token TABLE THEN TIES TIME TIMESTAMP TO
%token UNBOUNDED UNION UPDATE USING
%token VALUES VARCHAR VARYING VAR_POP VAR_SAMP
%token WHEN WHERE WITH WITHOUT
%token YEAR
%token ZONE TOP NVARCHAR USE TRIM UNPIVOT PIVOT DECLARE


%type <node> sql_stmt
%type <node> stmt_list
%type <node> stmt special_stmt
%type <node> dql_stmt
%type <node> dml_stmt
...
%type <node> large_object_length multiplier char_length_units data_type_len
...
%type <node> subquery row_subquery table_subquery asterisk_expr
%type <node> name_type reserved_type reserved_other  relation_factor_type  name_f reserved_func relation_factor_func
%type <node> opt_top_clause intnum_parens unpivot_clause pivot_clause
%type <node> ddl_stmt
%type <node> create_stmt create_table_stmt
%type <node> drop_stmt drop_table_stmt opt_if_exists_clause
%type <node> relation_factor_list
%type <node> sp_executesql_stmt
...
special_stmt:
    USE NAME
{
    $$ = Node::makeNonTerminalNode(E_USE, 1, $2);
    $$->serialize_format = &USE_SERIALIZE_FORMAT;
}
    | SET PARAM COMP_EQ expr_const
{
    std::string val = "SET ";
    val += $2->text();
    delete($2);
    ...
    | sp_executesql_stmt
{
    ...
    | EXEC sp_executesql_stmt
{
    $$ = $2;
}
    | EXECUTE sp_executesql_stmt
{
    $$ = $2;
}
;
...
{
    $$ = Node::makeNonTerminalNode(E_SP_EXECUTESQL, E_SP_EXECUTESQL_PROPERTY_CNT, $2);
    $$->serialize_format = &SP_EXECUTESQL_SERIALIZE_FORMAT;
}
;
...
create_table_stmt:
    CREATE TABLE relation_factor CREATE_TABLE_REST
{
    $$ = Node::makeNonTerminalNode(E_CREATE_TABLE, E_CREATE_TABLE_PROPERTY_CNT, $3, $4);
    $$->serialize_format = &CREATE_TABLE_SERIALIZE_FORMAT;
}
;

insert_stmt:
    INSERT  opt_top_clause INTO relation_factor insert_columns_and_source
{
    $$ = Node::makeNonTerminalNode(E_INSERT, 3, $4, $5, $2);
    $$->serialize_format = &INSERT_SERIALIZE_FORMAT_V3;
}
;
...
delete_stmt:
    DELETE opt_top_clause FROM relation_factor opt_where
{
...
drop_table_stmt:
    DROP TABLE opt_if_exists_clause relation_factor_list
{
    $$ = Node::makeNonTerminalNode(E_DROP_TABLE, E_DROP_TABLE_PROPERTY_CNT,
        $3, /* E_DROP_TABLE_OPT_IF_EXISTS */
        $4  /* E_DROP_TABLE_TABLE_LIST */);
    $$->serialize_format = &DROP_TABLE_SERIALIZE_FORMAT;
}
;

/* sql2003 support <update statement: searched> */
update_stmt:
    UPDATE opt_top_clause relation_factor SET update_elem_list opt_where
{
...
simple_table:
    SELECT opt_distinct opt_top_clause select_expr_list opt_from_clause opt_where opt_groupby opt_having
{
    $$ = Node::makeNonTerminalNode(E_SELECT, E_SELECT_PROPERTY_CNT,
  ...
  | table_subquery opt_as_label opt_simple_ident_list_with_parens
{
  ...
  | relation_factor opt_as_label unpivot_clause
{
    ...
    if ($2)
    {
      $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, select_with_parens_node, $2, $3, nullptr, nullptr);
    }
    else
    {
      Node* identifier_node = Node::makeTerminalNode(E_IDENTIFIER, $1->getChild(E_TABLE_IDENT_OBJECT)->text());
      $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, select_with_parens_node, identifier_node, $3, nullptr, nullptr);
    }
  ...
  | table_subquery opt_as_label unpivot_clause
{
  ...
  | relation_factor opt_as_label pivot_clause
{
    // Translate "relation_factor" into "table_subquery" node for mask and filter
    Node* alias_node = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, nullptr, nullptr, nullptr, nullptr);
    alias_node->serialize_format = &AS_SERIALIZE_FORMAT;
...
unpivot_clause:
    UNPIVOT '('  name_r FOR name_r IN simple_ident_list_with_parens ')' opt_as_label
{
...
pivot_clause:
    PIVOT '(' aggregate_windowed_function FOR name_r IN simple_ident_list_with_parens ')' opt_as_label
{
...
relation_factor_list:
    relation_factor
  | relation_factor_list ',' relation_factor
{
    $$ = Node::makeNonTerminalNode(E_RELATION_FACTOR_LIST, E_LIST_PROPERTY_CNT, $1, $3);
    $$->serialize_format = &COMMA_LIST_SERIALIZE_FORMAT;
}
;
...
scalar_function:
    relation_factor_func '('                      ')'
{
    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,
    		$1, nullptr, nullptr, nullptr, nullptr);
    $$->serialize_format = &FUN_CALL_1_OVER_SERIALIZE_FORMAT;
}
  | relation_factor_func '('            row_expr_list ')'
{
    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,
    		$1, $3, nullptr, nullptr, nullptr);
    $$->serialize_format = &FUN_CALL_1_OVER_SERIALIZE_FORMAT;
  ...
  |                            CAST '('    row_expr AS data_type ')'	/* sql2003 support <cast specification> */
{
  ...
  |                         CONVERT '(' data_type ','  row_expr ',' INTNUM ')'	/* sql2003 support <transcoding> */
{
    Node* fun_name = Node::makeTerminalNode(E_IDENTIFIER, "CONVERT");
    Node* sub_expr_list_node = Node::makeNonTerminalNode(E_EXPR_LIST, E_LIST_PROPERTY_CNT, $3, $5);
    sub_expr_list_node->serialize_format = &COMMA_LIST_SERIALIZE_FORMAT;
    Node* expr_list_node = Node::makeNonTerminalNode(E_EXPR_LIST, E_LIST_PROPERTY_CNT, sub_expr_list_node, $7);
    expr_list_node->serialize_format = &COMMA_LIST_SERIALIZE_FORMAT;

    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,	fun_name, expr_list_node, nullptr, nullptr, nullptr);
    $$->serialize_format = &FUN_CALL_BIGQUERY_S_FORMAT;
}
  |                         CONVERT '(' data_type ','  row_expr  ')'	/* sql2003 support <transcoding> */
{
    Node* fun_name = Node::makeTerminalNode(E_IDENTIFIER, "CONVERT");
    Node* expr_list_node = Node::makeNonTerminalNode(E_EXPR_LIST, E_LIST_PROPERTY_CNT, $3, $5);
    expr_list_node->serialize_format = &COMMA_LIST_SERIALIZE_FORMAT;
  ...
  |   TRIM '(' row_expr FROM  row_expr ')'
{
    ...
    $$->serialize_format = &FUNC_TRIM_SERIALIZE_FORMAT;
}
;
...
| VARCHAR
{
    $$ = Node::makeTerminalNode(E_STRING, "VARCHAR");
}
| NVARCHAR
{
    $$ = Node::makeTerminalNode(E_STRING, "NVARCHAR");
}
| NVARCHAR '(' data_type_len ')'
{
...
data_type_len:
INTNUM
| MAX  { $$ = Node::makeTerminalNode(E_STRING, "MAX"); }
...
reserved_other:
    ...
    | FILE_TABLE           { $$ = Node::makeTerminalNode(E_IDENTIFIER, "FileTable"); }
;
...
opt_if_exists_clause:
    /* EMPTY */ { $$ = nullptr; }
    | IF EXISTS { $$ = Node::makeTerminalNode(E_IDENTIFIER, "IF EXISTS"); };

opt_top_clause:
    /* EMPTY */	{ $$ = nullptr; }
|    TOP intnum_parens
{
    $$ = Node::makeTerminalNode(E_STRING, "TOP "+$2->text() );
    delete($2);
}
;
intnum_parens:
