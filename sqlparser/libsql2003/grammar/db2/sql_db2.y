...
%output  "sqlparser_db2_bison.cpp"
%defines "sqlparser_db2_bison.h"
...
// Prefix the parser
%define api.prefix {db2_}
%define api.token.prefix {DB2_}
...
%token YEAR
%token ZONE
%token  SKIP LOCKED DATA CS UR RS RR QUERYNO FETCH FIRST  OPTIMIZE LIMIT TABLE
%token DEGREE SCHEMA TIMEZONE
%token COUNT_BIG STDDEV
%token USE  KEEP EXCLUSIVE LOCKS CARDINALITY BIT MINUTES SHARE OUTCOME WAIT OFFSET
...
%type <node> from_list table_reference table_primary relation_factor relation_factor_datatype relation_factor_fuction
...
%type <node> name_r reserved  reserved_other reserved_type name_type name_fuc
%type <node> grouping_element_list grouping_element
%type <node> subquery row_subquery table_subquery
%type <node> opt_db2_special_clause_list db2_special_clause_list db2_special_clause
%type <node> db2_for_clause for_update read_only opt_of_simple_ident_list skip_locked with_isolation isolation  queryno_clause fetch_clause  optimize_clause limit_num opt_db2_locks
%type <node> opt_col_def_list_with_parens col_def_list col_def opt_cardinality param_num offset_num
...
insert_columns_and_source:
    				  select_stmt
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE_DB2, 3, nullptr, $1, nullptr);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  | simple_ident_list_with_parens select_stmt
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE_DB2, 3, $1, $2, nullptr);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  | simple_ident_list_with_parens from_constructor opt_db2_special_clause_list
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE_DB2, 3, $1, $2, $3);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  |                               from_constructor opt_db2_special_clause_list
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE_DB2, 3, nullptr, $1, $2);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  | DEFAULT VALUES
{
    Node* src = Node::makeTerminalNode(E_IDENTIFIER, "DEFAULT VALUES");
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE_DB2, 3, nullptr, src, nullptr);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
;
...
/* db2 support <delete statement: searched> */
delete_stmt:
    DELETE FROM relation_factor opt_where opt_db2_special_clause_list
{
...
/* db2 support <update statement: searched> */
update_stmt:
    UPDATE relation_factor SET update_elem_list opt_where opt_db2_special_clause_list
{
...
/* db2 support <direct select statement: multiple rows> */
select_stmt:
    query_expression opt_order_by  opt_db2_special_clause_list
{
    $$ = $1;
    $$->setChild(2, $2);
    $$->setChild(3, $3);
...
query_primary:
    simple_table
  | '(' select_stmt ')'	/* in db2 spec, here should be <query expression body> here generate a reduce/reduce error, however it works (all test passed) */
{
...
grouping_element:
    '(' ')'	{ $$ = Node::makeTerminalNode(E_IDENTIFIER,"()"); }	/* db2 support <empty grouping set> */
  | row_expr								/* db2 support <ordinary grouping set> <rollup list> <cube list> */
  | GROUPING SETS '(' grouping_element_list ')'				/* db2 support <grouping sets specification> */
{
...
/* db2 support RECURSIVE common table expression */
with_clause:
    WITH with_list
{
...
/*todo db2 support <search or cycle clause> */
common_table_expr:
...
/* todo db2 support LATERAL subquery */
/* todo db2 support <collection derived table> */
/* todo db2 support <table function derived table> */
/* todo db2 support <only spec> */
table_primary_non_join:
  ...
  | table_subquery opt_as_label opt_simple_ident_list_with_parens
{
 ...
 | TABLE '(' row_expr opt_cardinality  ')'  opt_as_label opt_col_def_list_with_parens
{
    $$ = Node::makeNonTerminalNode(E_DB2_CONSTRUCT_TABLE, 4,
        $3, $4, $6, $7);
    $$->serialize_format = &DB2_TABLE_SERIALIZE_FORMAT;
}
;
...
/* keep these that can solve <identifier chain> in db2 */
column_ref:
		 			    name_r
{
...
/* todo db2 support <similar predicate> */
/* todo db2 support <unique predicate> */
/* todo db2 support <normalized predicate> */
/* todo db2 support <match predicate> */
/* todo db2 support <overlaps predicate> */
/* todo db2 support <distinct predicate> */
/* todo db2 support <member predicate> */
/* todo db2 support <submultiset predicate> */
/* todo db2 support <set predicate> */
/* todo db2 support <type predicate> */
predicate:
...
/* db2 support <simple case> <searched case> */
case_expr:
...
case_arg:
    /*EMPTY*/	{ $$ = nullptr; }	/* db2 support <searched case> */
  | row_expr				/* db2 support <simple case> */
;
...
aggregate_function_name:
  ...
  | COUNT_BIG   { $$ = Node::makeTerminalNode(E_IDENTIFIER, "COUNT_BIG");}
    ...
    relation_factor_fuction '('                      ')'
{
  ...
  | relation_factor_fuction '('            row_expr_list ')'
{
    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,
    		$1, $3, nullptr, nullptr, nullptr);
    $$->serialize_format = &FUN_CALL_1_OVER_SERIALIZE_FORMAT;
...
/* todo function_call_keyword with db2 keyword */
  ...
  | LEFT '(' ')'
{
  ...
  |                            CAST '('    row_expr AS data_type ')'	/* db2 support <cast specification> */
{
  ...
  |                         CONVERT '(' row_expr USING relation_factor ')'	/* db2 support <transcoding> */
{
  ...
  |                        COALESCE '('            row_expr_list ')'	/* db2 support <case abbreviation> */
{
  ...
  |               CURRENT_TIMESTAMP					/* db2 support <current timestamp value function> */
{
  ...
  |               CURRENT_TIMESTAMP '('            INTNUM    ')'	/* db2 support <current timestamp value function> */
{
    Node* fun_name = Node::makeTerminalNode(E_IDENTIFIER, "CURRENT_TIMESTAMP");
    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,
    		fun_name, $3, nullptr, nullptr, nullptr);
  ...
  |                    CURRENT_USER					/* db2 support <general value specification> */
{
  ...
  |                        NULLIF '('          row_expr ',' row_expr ')'	/* db2 support <case abbreviation> */
{
  ...
  |                  SESSION_USER					/* db2 support <general value specification> */
{
  ...
  |                   SYSTEM_USER					/* db2 support <general value specification> */
{
  ...
  | VARCHAR '(' INTNUM ')'
{
  ...
  | VARCHAR '(' INTNUM ')' FOR BIT DATA
{
    $$ = Node::makeTerminalNode(E_STRING, "VARCHAR("+$3->text()+") FOR BIT DATA");
    delete($3);
}
...
label:
  ...
  | reserved
  | STRING
  | CASE                { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CASE"); }
  /*| FOR			{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "FOR"); } */		/* SQL-2003-R, here is non-reserved */
 /* | OPTIMIZE    { $$ = Node::makeTerminalNode(E_IDENTIFIER, "OPTIMIZE"); }*/
;
...
name_fuc:
    ...
    | TIMEZONE            { $$ = Node::makeTerminalNode(E_IDENTIFIER, "TIMEZONE"); }
    | BIT                 { $$ = Node::makeTerminalNode(E_IDENTIFIER, "BIT"); }
...
reserved:
    ...
    | reserved_other
    | ANY             { $$ = Node::makeTerminalNode(E_IDENTIFIER, "ANY"); }   /* not to function name*/
...
reserved_other:
    ...
    | EXCLUDE		{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "EXCLUDE"); }
    | ASC			{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "ASC"); }
    ...
    | CURRENT TIMESTAMP   { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CURRENT TIMESTAMP"); }
    | CURRENT DEGREE      { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CURRENT DEGREE"); }
    | CURRENT SCHEMA      { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CURRENT SCHEMA"); }
    | CURRENT TIMEZONE    { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CURRENT TIMEZONE"); }
    | DEGREE              { $$ = Node::makeTerminalNode(E_IDENTIFIER, "DEGREE"); }
    | SCHEMA              { $$ = Node::makeTerminalNode(E_IDENTIFIER, "SCHEMA"); }
    | LOCKED              { $$ = Node::makeTerminalNode(E_IDENTIFIER, "LOCKED"); }
    | DATA                { $$ = Node::makeTerminalNode(E_IDENTIFIER, "DATA"); }
    | USE                 { $$ = Node::makeTerminalNode(E_IDENTIFIER, "USE"); }
    | KEEP                { $$ = Node::makeTerminalNode(E_IDENTIFIER, "KEEP"); }
    | EXCLUSIVE           { $$ = Node::makeTerminalNode(E_IDENTIFIER, "EXCLUSIVE"); }
    | LOCKS               { $$ = Node::makeTerminalNode(E_IDENTIFIER, "LOCKS"); }
    | CARDINALITY         { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CARDINALITY"); }
    | MINUTES             { $$ = Node::makeTerminalNode(E_IDENTIFIER, "MINUTES"); }
    | SHARE               { $$ = Node::makeTerminalNode(E_IDENTIFIER, "SHARE"); }
    | WAIT                { $$ = Node::makeTerminalNode(E_IDENTIFIER, "WAIT"); }
    | OUTCOME             { $$ = Node::makeTerminalNode(E_IDENTIFIER, "OUTCOME"); }
...
opt_db2_special_clause_list:
    /*EMPTY*/ {$$ = nullptr;}
    | db2_special_clause_list
;
db2_special_clause_list:
db2_special_clause
| db2_special_clause  db2_special_clause_list
{
    $$ = Node::makeNonTerminalNode(E_SPECIAL_CLAUSE_LIST, 2, $1, $2);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
;
db2_special_clause:
...
for_update:
     FOR UPDATE opt_of_simple_ident_list
{
    Node * for_ = Node::makeTerminalNode(E_STRING, "FOR");
    Node * update_ = Node::makeTerminalNode(E_STRING, "UPDATE");
    $$ = Node::makeNonTerminalNode(E_FOR_UPDATE, 3, for_, update_, $3, NULL);
    $$->serialize_format = &FOR_UPDATE_FORMAT;
}
;
opt_of_simple_ident_list:
    /*EMPTY*/ {$$ = nullptr;}
    | OF simple_ident_list
{
    $$ = $2;
}

read_only:
	FOR READ ONLY { $$ = Node::makeTerminalNode(E_STRING, "FOR READ ONLY"); }
	| FOR FETCH ONLY { $$ = Node::makeTerminalNode(E_STRING, "FOR FETCH ONLY"); }

skip_locked:
	SKIP LOCKED DATA { $$ = Node::makeTerminalNode(E_STRING, "SKIP LOCKED DATA"); }

with_isolation:
	WITH isolation opt_db2_locks
{
     std::string ss = "WITH ";
     ss += $2->text() ;
     delete($2);
...
isolation:
    CS { $$ = Node::makeTerminalNode(E_IDENTIFIER, "CS"); }
    | UR { $$ = Node::makeTerminalNode(E_IDENTIFIER, "UR"); }
    | RS { $$ = Node::makeTerminalNode(E_IDENTIFIER, "RS"); }
    | RR { $$ = Node::makeTerminalNode(E_IDENTIFIER, "RR"); }
opt_db2_locks:
    /*EMPTY*/ {$$ = nullptr;}
    | USE AND KEEP EXCLUSIVE LOCKS    { $$ = Node::makeTerminalNode(E_STRING, " USE AND KEEP EXCLUSIVE LOCKS"); }
    | USE AND KEEP UPDATE LOCKS    { $$ = Node::makeTerminalNode(E_STRING, " USE AND KEEP UPDATE LOCKS"); }
    | USE AND KEEP SHARE LOCKS    { $$ = Node::makeTerminalNode(E_STRING, " USE AND KEEP SHARE LOCKS"); }
    | KEEP LOCKS  { $$ = Node::makeTerminalNode(E_STRING, " KEEP LOCKS"); }
    | WAIT FOR OUTCOME { $$ = Node::makeTerminalNode(E_STRING, " WAIT FOR OUTCOME"); }
    | USE AND KEEP UPDATE LOCKS WAIT FOR OUTCOME   { $$ = Node::makeTerminalNode(E_STRING, " USE AND KEEP UPDATE LOCKS WAIT FOR OUTCOME"); }
;

queryno_clause:
	QUERYNO param_num { $$ = Node::makeTerminalNode(E_STRING, "QUERYNO "+$2->text()); delete($2); }
fetch_clause:
	FETCH FIRST param_num  ROWS  ONLY       { $$ = Node::makeTerminalNode(E_STRING, "FETCH FIRST "+$3->text()+" ROWS ONLY"); delete($3);  }
	| FETCH FIRST param_num  ROW  ONLY      { $$ = Node::makeTerminalNode(E_STRING, "FETCH FIRST "+$3->text()+" ROW ONLY"); delete($3);  }
	| FETCH FIRST param_num  ROWS           { $$ = Node::makeTerminalNode(E_STRING, "FETCH FIRST "+$3->text()+" ROWS"); delete($3);  }
	| FETCH FIRST param_num  ROW            { $$ = Node::makeTerminalNode(E_STRING, "FETCH FIRST "+$3->text()+" ROW"); delete($3);  }
optimize_clause:
    OPTIMIZE FOR param_num  ROWS  ONLY       { $$ = Node::makeTerminalNode(E_STRING, "OPTIMIZE FOR "+$3->text()+" ROWS ONLY"); delete($3);  }
    | OPTIMIZE FOR param_num  ROW  ONLY      { $$ = Node::makeTerminalNode(E_STRING, "OPTIMIZE FOR "+$3->text()+" ROW ONLY"); delete($3);  }
    | OPTIMIZE FOR param_num  ROWS           { $$ = Node::makeTerminalNode(E_STRING, "OPTIMIZE FOR "+$3->text()+" ROWS"); delete($3);  }
    | OPTIMIZE FOR param_num  ROW            { $$ = Node::makeTerminalNode(E_STRING, "OPTIMIZE FOR "+$3->text()+" ROW"); delete($3);  }
...
col_def:
  value_expression data_type
{
    $$ = Node::makeNonTerminalNode(E_COLUMN_DEFINED, 2, $1, $2);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}

opt_cardinality:
    /*EMPTY*/ {$$ = nullptr;}
    | CARDINALITY param_num   { $$ = Node::makeTerminalNode(E_STRING, "CARDINALITY "+ $2->text()); delete($2); }
