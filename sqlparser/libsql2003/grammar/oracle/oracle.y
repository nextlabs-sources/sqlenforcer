...
%output  "sqlparser_oracle_bison.cpp"
%defines "sqlparser_oracle_bison.h"
...
// Prefix the parser
%define api.prefix {oracle_}
%define api.token.prefix {ORACLE_}
...
%token SCOPE SECOND SELECT SESSION_USER SET SETS SMALLINT
       SOME STDDEV STDDEV_POP STDDEV_SAMP SUM SYSTEM_USER
...
%token ZONE
%token RETURNING
%token SKIP LOCKED NOWAIT WAIT NULLS LAST SIBLINGS
%token OFFSET FIRST NEXT  FETCH PERCENT MINUS UNIQUE NAN INFINITE
%token REGEXP_LIKE APPLY
%token CYCLE SEARCH DEPTH BREADTH START CONNECT NOCYCLE
%token CONTAINERS SHARDS SAMPLE BLOCK SEED PRIOR ALTER SESSION
%token BEGIN DECLARE UNPIVOT PIVOT EXTRACT TIMEZONE_HOUR  TIMEZONE_MINUTE TIMEZONE_REGION TIMEZONE_ABBR


%type <node> sql_stmt body_stmt
%type <node> stmt_list
%type <node> stmt
%type <node> dql_stmt
%type <node> dml_stmt
...
%type <node> select_expr_list select_expr_list_with_parens
%type <node> opt_where opt_groupby opt_order_by order_by opt_having  opt_for_update
%type <node> sort_list sort_key opt_asc_desc
%type <node> opt_distinct distinct_or_all projection
%type <node> from_list table_reference table_primary relation_factor relation_factor_2
%type <node> joined_table cross_join qualified_join natural_join cross_apply
%type <node> join_type
%type <ival> join_outer
%type <node> expr_const
%type <node> search_condition boolean_term boolean_factor boolean_test boolean_primary truth_value null_value
...
%type <node> subquery row_subquery table_subquery cte_subquery
%type <node> opt_returning_into variable variable_list
%type <node> for_update opt_of_column_ref_list  column_ref_list opt_wait_nowait opt_ignore_lock opt_nulls_c
%type <node> row_limiting_clause opt_offset_clause opt_fetch_rows_clause first_next only_or_ties variable_v2 opt_variable_v2 row_rows bool_function
%type <node> opt_cycle_clause opt_search_clause cycle_clause search_clause hierarchical_query_clause  opt_hierarchical_query_clause connect_by connect_opt start_with
%type <node> table_function sample_clause opt_seed_value partition_by
%type <node> name_type reserved_type reserved_other reserved_non_type relation_factor_type asterisk_expr simple_table_alias special_stmt
%type <node> unpivot_clause pivot_clause
%start sql_stmt
...
body_stmt:
    BEGIN stmt_list  END
{
    $$ = Node::makeNonTerminalNode(E_STMT_BODYS, 1, $2);
    $$->serialize_format = &STMT_BODYS_SERIALIZE_FORMAT_ORA;
}
;
special_stmt:
    ALTER SESSION SET NAME COMP_EQ NAME
{
    $$ = Node::makeNonTerminalNode(EORA_ALTER_SESSION, 2, $4, $6);
    $$->serialize_format = &ALTER_SESSION_SERIALIZE_FORMAT_ORA;
}
    | SET NAME COMP_EQ expr_const
{
    std::string val = "SET ";
    val += $2->text();
    delete($2);
...
insert_columns_and_source:
    				  select_stmt opt_returning_into
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE, 3, nullptr, $1, $2);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  | simple_ident_list_with_parens select_stmt opt_returning_into
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE, 3, $1, $2, $3);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  | simple_ident_list_with_parens from_constructor opt_returning_into
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE, 3, $1, $2, $3);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
  |                               from_constructor opt_returning_into
{
    $$ = Node::makeNonTerminalNode(E_INSERT_COLUMNS_AND_SOURCE, 3, nullptr, $1, $2);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;
}
...
delete_stmt:
    DELETE FROM simple_table_alias opt_where
{
...
update_stmt:
    UPDATE simple_table_alias SET update_elem_list opt_where  opt_returning_into
{
    $$ = Node::makeNonTerminalNode(E_UPDATE, E_UPDATE_PROPERTY_CNT,
    		...
    		$6		/* E_UPDATE_OPT_QUERY_HINT 8 */);
    $$->serialize_format = &UPDATE_SERIALIZE_FORMAT;
}
;
opt_returning_into:
/*empty*/ {$$ = NULL;}
    | RETURNING row_expr_list INTO variable_list
{
    $$ = Node::makeNonTerminalNode(EORA_RETURNING_INTO, 2, $2, $4);
    $$->serialize_format = &RETURNING_INTO_SERIALIZE_FORMAT;
}
;
variable_list:
    variable
...
{
    $$ = Node::makeNonTerminalNode(E_VAR_LIST, E_LIST_PROPERTY_CNT, $1, $3);
    $$->serialize_format = &COMMA_LIST_SERIALIZE_FORMAT;
}
;
variable:
...
query_expression:
    query_expression_body opt_order_by opt_for_update row_limiting_clause
{
    $$ = Node::makeNonTerminalNode(E_DIRECT_SELECT, E_DIRECT_SELECT_PROPERTY_CNT, nullptr, $1, nullptr, nullptr,nullptr);
    $$->serialize_format = &SELECT_DIRECT_SERIALIZE_FORMAT;

        $$->setChild(E_DIRECT_SELECT_ORDER, $2);
        $$->setChild(E_DIRECT_SELECT_FOR_UPDATE, $3);
        $$->setChild(E_DIRECT_HINT_CLAUSE, $4);

}
  | with_clause query_expression_body opt_order_by opt_for_update row_limiting_clause
{
    $$ = Node::makeNonTerminalNode(E_DIRECT_SELECT, E_DIRECT_SELECT_PROPERTY_CNT, $1, $2, nullptr, nullptr,nullptr);
    $$->serialize_format = &SELECT_DIRECT_SERIALIZE_FORMAT;


        $$->setChild(E_DIRECT_SELECT_ORDER, $3);
        $$->setChild(E_DIRECT_SELECT_FOR_UPDATE, $4);
        $$->setChild(E_DIRECT_HINT_CLAUSE, $5);
}
...
simple_table:
    SELECT opt_distinct select_expr_list opt_from_clause opt_where opt_hierarchical_query_clause opt_groupby opt_having
{
    $$ = Node::makeNonTerminalNode(E_SELECT, E_SELECT_PROPERTY_CNT,
    ...
    $$->serialize_format = &SELECT_SERIALIZE_FORMAT_ORA;
}
    | SELECT UNIQUE opt_distinct select_expr_list opt_from_clause opt_where opt_hierarchical_query_clause opt_groupby opt_having
{
    $$ = Node::makeNonTerminalNode(E_SELECT, E_SELECT_PROPERTY_CNT,
    ...
    $$->serialize_format = &SELECT_SERIALIZE_FORMAT2_ORA;
}
;
...
{
    $$ = Node::makeNonTerminalNode(E_ORDER_BY, E_ORDER_BY_PROPERTY_CNT, $4, nullptr);
    $$->serialize_format = &ORDER_SIBLINGS_BY_SERIALIZE_FORMAT;
}
;
...
sort_key:
    row_expr opt_asc_desc opt_nulls_c
{
...
common_table_expr:
    name_r opt_derived_column_list AS cte_subquery  opt_search_clause opt_cycle_clause
{
    $$ = Node::makeNonTerminalNode(E_COMMON_TABLE_EXPR, 5, $1, $2, $4, $5, $6);
    $$->serialize_format = &COMMON_TABLE_EXPR_SERIALIZE_FORMAT_ORA;
}
;
  ...
  | table_subquery opt_as_label opt_simple_ident_list_with_parens
{
  ...
  | table_function opt_as_label
{
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, $2, nullptr, nullptr, nullptr);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
  | relation_factor_2  sample_clause opt_as_label
{
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, $3, nullptr, nullptr, $2);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
   | relation_factor_2 opt_as_label partition_by
{
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, $2, nullptr, $3, nullptr);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
  | relation_factor_2 opt_as_label unpivot_clause
{
    // Translate "relation_factor" into "table_subquery" node for mask and filter
    Node* alias_node = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, nullptr, nullptr, nullptr, nullptr);
    alias_node->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
    ...
    if ($2){
        ...
        alias_name_node =  Node::makeTerminalNode(E_IDENTIFIER, $1->getChild(E_TABLE_IDENT_OBJECT)->text());
    }
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, select_with_parens_node, alias_name_node, $3, nullptr, nullptr);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
  | table_subquery opt_as_label unpivot_clause
{
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, $2, $3, nullptr, nullptr);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
  | relation_factor_2 opt_as_label pivot_clause
{
    // Translate "relation_factor" into "table_subquery" node for mask and filter
    Node* alias_node = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, nullptr, nullptr, nullptr, nullptr);
    alias_node->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
  ...
  | table_subquery opt_as_label pivot_clause
{
    $$ = Node::makeNonTerminalNode(E_ALIAS, E_ALIAS_PROPERTY_CNT, $1, $2, $3, nullptr, nullptr);
    $$->serialize_format = &ORA_AS_SERIALIZE_FORMAT;
}
;
unpivot_clause:
    UNPIVOT '('  name_r FOR name_r IN select_expr_list_with_parens ')' opt_as_label
{
    Node* npvt = Node::makeTerminalNode(E_IDENTIFIER, "UNPIVOT");
    $$ = Node::makeNonTerminalNode(E_UNPIVOT_PIVOT, 5, npvt, $3, $5, $7, $9);
    $$->serialize_format = &PIVOT_CLAUSE_SERIALIZE_FORMAT;
}
;
pivot_clause:
    PIVOT '(' aggregate_windowed_function FOR name_r IN select_expr_list_with_parens ')' opt_as_label
{
...
{
    $$ = Node::makeNonTerminalNode(EORA_PARTITION_BY, 1, $4);
    $$->serialize_format = &PARTITION_BY_SERIALIZE_FORMAT;
}
;
table_function:
    ONLY '(' relation_factor ')'
{
...
cross_apply:
    table_reference CROSS APPLY table_primary
{
    ...
    | table_reference OUTER APPLY table_primary
{
...
{
    $$ = Node::makeNonTerminalNode(E_OP_LIKE, E_OP_TERNARY_PROPERTY_CNT, $1, $3, NULL);
...
null_predicate:
    row_expr IS null_value
{
    $$ = Node::makeNonTerminalNode(E_OP_IS, E_OP_BINARY_PROPERTY_CNT, $1, $3);
    $$->serialize_format = Node::op_serialize_format(E_OP_IS);
}
  | row_expr IS NOT null_value
{
    $$ = Node::makeNonTerminalNode(E_OP_IS_NOT, E_OP_BINARY_PROPERTY_CNT, $1, $4);
    $$->serialize_format = Node::op_serialize_format(E_OP_IS_NOT);
}
...
null_value:
    NULLX
   | NAN     { $$ = Node::makeTerminalNode(E_IDENTIFIER, "NAN"); }
   | INFINITE  { $$ = Node::makeTerminalNode(E_IDENTIFIER, "INFINITE"); }
  ...
  |                   SYSTEM_USER					/* sql2003 support <general value specification> */
{
    ...
    | EXTRACT '(' end_field FROM row_expr')'
{
    Node* fun_name = Node::makeTerminalNode(E_IDENTIFIER, "EXTRACT");
    $$ = Node::makeNonTerminalNode(E_FUN_CALL, E_FUN_CALL_PROPERTY_CNT,
            fun_name, $5, $3, nullptr, nullptr);
    $$->serialize_format = &FUN_EXTRACT_SERIALIZE_FORMAT_ORA;
}
;
  ...
  | MINUTE
{
    ...
    | TIMEZONE_HOUR
{
    $$ = Node::makeTerminalNode(E_STRING, "TIMEZONE_HOUR");
}
    | TIMEZONE_MINUTE
{
    $$ = Node::makeTerminalNode(E_STRING, "TIMEZONE_MINUTE");
}
    | TIMEZONE_REGION
{
    $$ = Node::makeTerminalNode(E_STRING, "TIMEZONE_REGION");
}
    | TIMEZONE_ABBR
{
    $$ = Node::makeTerminalNode(E_STRING, "TIMEZONE_ABBR");
}
;
...
reserved_type:
    ...
    //| BFILE 	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "BFILE"); }
    | CLOB  	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "CLOB"); }
    | BLOB 	    { $$ = Node::makeTerminalNode(E_IDENTIFIER, "BLOB"); }
    | NCLOB 	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "NCLOB"); }
    | NUMERIC	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "NUMERIC"); }
    | CHARACTER	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "CHARACTER"); }
    | BIGINT	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "BIGINT"); }
    | DEC	    { $$ = Node::makeTerminalNode(E_IDENTIFIER, "DEC"); }
    | NATIONAL	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "NATIONAL"); }
    | DOUBLE	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "DOUBLE"); }
    | VARYING	{ $$ = Node::makeTerminalNode(E_IDENTIFIER, "VARYING"); }
...
reserved_other:
    ...
    | SAMPLE  { $$ = Node::makeTerminalNode(E_IDENTIFIER, "SAMPLE"); }
    | SESSION { $$ = Node::makeTerminalNode(E_IDENTIFIER, "SESSION"); }
    | BEGIN  { $$ = Node::makeTerminalNode(E_IDENTIFIER, "BEGIN"); }
    | DECLARE  { $$ = Node::makeTerminalNode(E_IDENTIFIER, "DECLARE"); }
;
...
for_update:
     FOR UPDATE opt_of_column_ref_list opt_wait_nowait opt_ignore_lock
{
    $$ = Node::makeNonTerminalNode(E_FOR_UPDATE, 3, $3, $4, $5);
    $$->serialize_format = &FOR_UPDATE_FORMAT_HANA;
}
;
opt_of_column_ref_list:
    /*EMPTY*/ {$$ = nullptr;}
    | OF column_ref_list
{
...
opt_ignore_lock:
    /*EMPTY*/ {$$ = nullptr;}
    | SKIP LOCKED { $$ = Node::makeTerminalNode(E_STRING, "SKIP LOCKED");}
;
opt_wait_nowait:
    /*EMPTY*/ {$$ = nullptr;}
...
;
row_limiting_clause:
opt_offset_clause opt_fetch_rows_clause
{
    $$ = Node::makeNonTerminalNode(EORA_OFFSET_FETCH, 2,  $1, $2);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
;
row_rows:
ROW  { $$ = Node::makeTerminalNode(E_STRING, "ROW");}
| ROWS  { $$ = Node::makeTerminalNode(E_STRING, "ROWS");}
;
opt_variable_v2:
    /*EMPTY*/ {$$ = nullptr;}
    | variable_v2
...
opt_offset_clause:
    /*EMPTY*/ {$$ = nullptr;}
    | OFFSET variable_v2 row_rows
{
    $$ = Node::makeNonTerminalNode(EORA_OFFSET_ROW, 2,  $2, $3);
    $$->serialize_format = &OFFSET_SERIALIZE_FORMAT;
}
;

first_next:
    FIRST  { $$ = Node::makeTerminalNode(E_STRING, "FIRST");}
    |NEXT  { $$ = Node::makeTerminalNode(E_STRING, "NEXT");}

only_or_ties:
    ONLY  { $$ = Node::makeTerminalNode(E_STRING, "ONLY");}
    | WITH TIES { $$ = Node::makeTerminalNode(E_STRING, "WITH TIES");}

opt_fetch_rows_clause:
    /*EMPTY*/ {$$ = nullptr;}
    | FETCH first_next opt_variable_v2 row_rows only_or_ties
{
    Node * name1 = nullptr;
    {
        std::string name = "FETCH "+$2->text();
        delete($2);
        name1 = Node::makeTerminalNode(E_STRING, name);
    }
    Node * name2 = nullptr;
    {
        std::string name = $4->text()+" "+$5->text();
        delete($4);
        delete($5);
        name2 = Node::makeTerminalNode(E_STRING, name);
    }
    $$ = Node::makeNonTerminalNode(EORA_OFFSET_ROW, 3, name1, $3, name2);
    $$->serialize_format = &TRIPLE_SERIALIZE_FORMAT;

}
    | FETCH first_next opt_variable_v2 PERCENT row_rows  only_or_ties
{
    Node * name1 = nullptr;
    {
        std::string name = "FETCH "+$2->text();
    ...
    Node * name2 = nullptr;
    {
        std::string name =" PERCENT "+ $5->text()+" "+$6->text();
        delete($5);
        delete($6);
        name2 = Node::makeTerminalNode(E_STRING, name);
    }
...
;
search_clause:
    SEARCH DEPTH  FIRST BY simple_ident_list opt_asc_desc opt_nulls_c SET name_r
{
    std::string name = "SEARCH DEPTH FIRST BY ";
    Node * prename  = Node::makeTerminalNode(E_STRING, name);
    $$ = Node::makeNonTerminalNode(EROA_SEARCH_CLAUSE, 5, prename, $5, $6, $7, $9);
    $$->serialize_format = &SEARCH_CLAUSE_SERIALIZE_FORMAT_ORA;
}
    | SEARCH BREADTH  FIRST BY simple_ident_list opt_asc_desc opt_nulls_c SET name_r
{
...
cycle_clause:
    CYCLE simple_ident_list SET name_r TO STRING DEFAULT STRING
{
    $$ = Node::makeNonTerminalNode(EORA_CYCLE_CLAUSE, 4, $2, $4, $6, $8);
    $$->serialize_format = &CYCLE_CLAUSE_SERIALIZE_FORMAT_ORA;
}
;
connect_opt:
/*EMPTY*/ {$$ = nullptr;}
| NOCYCLE  {$$ = Node::makeTerminalNode(E_STRING, "NOCYCLE"); }
| NOCYCLE PRIOR  {$$ = Node::makeTerminalNode(E_STRING, "NOCYCLE PRIOR"); }
| PRIOR  {$$ = Node::makeTerminalNode(E_STRING, "PRIOR"); }
;
connect_by:
    CONNECT BY connect_opt search_condition
{
    ...
    Node * prename  = Node::makeTerminalNode(E_STRING, name);
    $$ = Node::makeNonTerminalNode(EORA_CONNECT_BY, 2, prename, $4);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
;
start_with:
    START WITH search_condition
{
    ...
    Node * prename  = Node::makeTerminalNode(E_STRING, name);
    $$ = Node::makeNonTerminalNode(EORA_START_WITH, 2, prename, $3);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
;
hierarchical_query_clause:
...
{
    $$ = Node::makeNonTerminalNode(EORA_HIERARCHICAL_CLAUSE, 2, $1, $2);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
    | connect_by
{
    $$ = Node::makeNonTerminalNode(EORA_HIERARCHICAL_CLAUSE, 2, $1, nullptr);
    $$->serialize_format = &DOUBLE_SERIALIZE_FORMAT;
}
...
sample_clause:
    SAMPLE BLOCK '(' INTNUM ')' opt_seed_value
{
    Node * name = Node::makeTerminalNode(E_STRING, "SAMPLE BLOCK");
    $$ = Node::makeNonTerminalNode(EORA_SAMPLE_CLAUSE, 3, name, $4, $6);
    $$->serialize_format = &SAMPLE_CLAUSE_SERIALIZE_FORMAT;
}
    | SAMPLE '(' INTNUM ')' opt_seed_value
{
    Node * name = Node::makeTerminalNode(E_STRING, "SAMPLE");
    $$ = Node::makeNonTerminalNode(EORA_SAMPLE_CLAUSE, 3, name, $3, $5);
    $$->serialize_format = &SAMPLE_CLAUSE_SERIALIZE_FORMAT;
}
;
...
opt_seed_value:
    ...
    | SEED '(' INTNUM ')'
{
   $$ = Node::makeNonTerminalNode(EORA_SEED_VALUE, 1, $3);
   $$->serialize_format = &EORA_SEED_VALUE_SERIALIZE_FORMAT;
}
;
