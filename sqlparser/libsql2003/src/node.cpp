...
void parser_bigquery_tbname(Node* pnode, std::string & db, std::string & schema, std::string & tb){
    if (pnode == nullptr) return;
    if (pnode->nodeType_ == E_TABLE_IDENT) { ///E_OP_NAME_FIELD
        //ret._type = pnode->nodeType_;
        Node * ptb = pnode->getChild(0);
        Node * pschema = pnode->getChild(1);
        Node * pdbname = pnode->getChild(2);

        if(pdbname) {
            if(pschema) schema = pschema->terminalToken_.str;
            if(ptb){
                std::string table_name = ptb->terminalToken_.str;
                std::string table_name2 = ptb->terminalToken_.yytex;
                if(table_name == table_name2){ //if no double quote or back quote
            ...
            std::string schema_name = pschema->terminalToken_.str;
            std::string table_name = ptb->terminalToken_.str;
            std::string table_name2 = ptb->terminalToken_.yytex;
            if(table_name == table_name2){ //if no double quote or back quote
                transform(table_name.begin(), table_name.end(), table_name.begin(), ::toupper);
            ...
            std::size_t ifind2 = schema_name.find('.');
            if(ifind != std::string::npos){
                db = schema_name;
                schema = table_name.substr(0, ifind);
                tb = table_name.substr(ifind+1);
            }
            //`db.dataset`.tb
            else if (ifind2 != std::string::npos){
                db = schema_name.substr(0, ifind2);
                schema = schema_name.substr(ifind2+1);
                tb = table_name;
            } else {
                schema = schema_name;
                tb = table_name;
