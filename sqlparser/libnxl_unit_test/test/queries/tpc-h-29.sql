-- query worker top 2 <marked error : top clause in SQLServer>
/*SELECT TOP 2 *
FROM worker;*/
-- query worker top 50% <marked error : top clause in SQLServer>
...
FROM worker
WHERE wsex='女';
-- Query the salary table for the first two highest-paid employees <marked error : top clause in SQLServer>
/*SELECT TOP 2 *
FROM salary INNER JOIN worker
ON salary.wid=worker.wid
ORDER BY actualsalary DESC;*/
-- Query the top 18% of the total salary output in the salary table <marked error : top clause in SQLServer>
...
-- <marked error : special column alias syntax in SQLServer>
/*
...
-- <marked error : with hint clause in SQLServer>
/*
