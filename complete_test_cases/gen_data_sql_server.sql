...
INSERT INTO SA.CUSTOMER VALUES ('1321eqweasdsa12312', 'retail' , 0), ('12313798ada1321233', 'retail', 0);
UPDATE SA.CUSTOMER SET c_privilege_level = c_privilege_level - 1 WHERE c_mktsegment = 'building';
DELETE FROM    SA.CUSTOMER WHERE  c_mktsegment = 'FMCG';
...
SELECT o_orderpriority, COUNT(*) AS ORDER_COUNT FROM sa.orders WHERE o_orderdate >= '1983-07-01' AND o_orderdate < Dateadd(mm, 3, CAST('1993-07-01' AS datetime)) AND EXISTS (SELECT * FROM sa.customer WHERE c_custkey = o_custkey ) GROUP BY o_orderpriority ORDER BY o_orderpriority  ;
