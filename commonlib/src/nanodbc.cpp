...
#endif
#pragma warning(disable : 4996 5045 26812) // warning about deprecated declaration
#endif

#include "nanodbc.h"
#include "odbc_mgr_ori.h"
...
#define NANODBC_ODBC_VERSION SQL_OV_ODBC3    /* v3.8 error [unixODBC][Driver Manager]Invalid attribute value */
    ...
    {                                                                                              \
        std::cerr << __FILE__                                                                      \
            ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE(FUNC) "(" #__VA_ARGS__ ")"       \
                  << std::endl;                                                                    \
        RC = ODBC_MGR(ODBC_MGR_INS,.)##FUNC(__VA_ARGS__);                                                                    \
    } while (false) /**/
#define NANODBC_CALL(FUNC, ...)                                                                    \
    ...
    {                                                                                              \
        std::cerr << __FILE__                                                                      \
            ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE(FUNC) "(" #__VA_ARGS__ ")"       \
                  << std::endl;                                                                    \
        ODBC_MGR(ODBC_MGR_INS,.)##FUNC(__VA_ARGS__);                                                                         \
    } while (false) /**/
#else
#define NANODBC_CALL_RC(FUNC, RC, ...) RC = ODBC_MGR_INS.FUNC(__VA_ARGS__)
#define NANODBC_CALL(FUNC, ...) ODBC_MGR_INS.FUNC(__VA_ARGS__)
#endif
