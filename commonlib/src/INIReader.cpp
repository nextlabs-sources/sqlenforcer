...
string INIReader::Get(const string& section, const string& name, const string& default_value) const
{
    ...
    return 1==_values.count(key) ? _values.find(key)->second : default_value;
}
...
long INIReader::GetInteger(const string& section, const string& name, long default_value) const
{
    ...
    if (end != value + valstr.length()) {
        string msg = "Invalid value! Expect number value. [" + section + "] ---> " + name + ".";
...
bool INIReader::GetBoolean(const string& section, const string& name, bool default_value) const
{
    ...
    if (valstr == "true" || valstr == "yes" || valstr == "on" || valstr == "1")
        ...
        string msg = "Invalid value! Expect boolean value. [" + section + "] ---> " + name;
        throw std::runtime_error(msg.c_str());
    }
}
