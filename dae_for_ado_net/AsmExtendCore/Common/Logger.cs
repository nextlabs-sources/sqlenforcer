...
namespace AsmExtendCore
{
    static class Logger
    {
        ...
        public static void WriteLine(Exception ex)
        {
            try
            {
                StackTrace obExFullStack = new StackTrace(ex, true);
                Logger.WriteLine("Exception message:[{0}], stack:[{1}]\n", ex.Message, obExFullStack.ToString() /*ex.StackTrace*/);
                if (null != ex.InnerException)
                {
                    StackTrace obInnerExFullStack = new StackTrace(ex, true);
                    Logger.WriteLine("Inner exception message:[{0}], stack:[{1}]\n", ex.InnerException.Message, obInnerExFullStack.ToString() /*ex.InnerException.StackTrace*/);
                }
            }
            catch
            {
        ...
        {
            string strLogRootPathRet = AsmExtendBaseInfo.g_kstrAsmExtendDefaultRoot + "Logs\\";
            try
            {
