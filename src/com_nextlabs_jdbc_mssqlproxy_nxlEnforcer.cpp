#include "com_nextlabs_jdbc_mssqlproxy_nxlEnforcer.h"
...
JNIEXPORT jboolean JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerInit(JNIEnv *env, jobject obj,\
...
JNIEXPORT jlong JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerNewContext(JNIEnv *env, jobject obj, jstring userID,\
...
JNIEXPORT void JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerFreeContext(JNIEnv *env, jobject, jlong ctx)
{
...
// JNIEXPORT jobject JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerEvaluation(JNIEnv *env, jobject obj, \
...
//     jclass cls = env->FindClass("com/nextlabs/jdbc/mssqlproxy/nxlEnforcedSqlResult"); 
...
JNIEXPORT jobject JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerEvaluationV2(JNIEnv *env, jobject obj, \
jlong context, jstring sqlText, jobject meta, jobject excobj)
{
    ...
    if(exc.IsBreak()){
        //jclass _class = env->GetObjectClass(obj);
        //jfieldID exc_id = env->GetStaticFieldID(_class, "exception", "Lcom/nextlabs/jdbc/mssqlproxy/nxlSqlException;");
        //jobject exc_obj = env->GetStaticObjectField(_class,exc_id);  
        jclass exc_class = env->GetObjectClass(excobj);  
        jmethodID exc_setinfo = env->GetMethodID(exc_class,"SetInfo","(ILjava/lang/String;)V"); 
...
JNIEXPORT void JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerUserContext(JNIEnv *env, jobject, jlong enforcerContext, jint contextType, jstring contextInfo)
{
...
JNIEXPORT jint JNICALL Java_com_nextlabs_jdbc_mssqlproxy_nxlEnforcer_sqlEnforcerWriteLog(JNIEnv *env, jobject, jint level, jstring message)
{
    const char* msg = env->GetStringUTFChars(message,0);
    int result = SQLEnforcerWriteLogWithTagA((int)level, "MSSQL JDBC", msg);
