...
#ifdef WIN32
#include <windows.h>
#define GET_ODBC_PROC(funname) this->funname=(funname##Fun)::GetProcAddress(hODBCMgr, #funname);
#else
#include <dlfcn.h>
#include <sys/stat.h>
...
private:
	void init() {
        ...
        if (m == 0) {
            ...
            //hODBCMgr = dlopen("/usr/nextlabs/emdb/ODBC/libodbc_nxl.so", RTLD_NOW|RTLD_GLOBAL);
            //hODBCMgr = dlopen("/home/jeff/emdb_so/Common/bin64/libodbc_nxl.so", RTLD_NOW|RTLD_GLOBAL);
            //ODBCMgr = dlopen("/usr/nextlabs/emdb/ODBC/libodbc_nxl.so", RTLD_NOW|RTLD_GLOBAL);
        }
#endif
