...
// Fix error: [unixODBC][Driver Manager]Invalid attribute value
// According to https://github.com/nanodbc/nanodbc/issues/114
    ...
    {                                                                                              \
        std::cerr << __FILE__                                                                      \
            ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE(FUNC) "(" #__VA_ARGS__ ")"       \
                  << std::endl;                                                                    \
        RC = ODBC_MGR(odbcapis_nxl_ins,.)##FUNC(__VA_ARGS__);                                                                    \
    } while (false) /**/
#define NANODBC_CALL(FUNC, ...)                                                                    \
    ...
    {                                                                                              \
        std::cerr << __FILE__                                                                      \
            ":" NANODBC_STRINGIZE(__LINE__) " " NANODBC_STRINGIZE(FUNC) "(" #__VA_ARGS__ ")"       \
                  << std::endl;                                                                    \
        ODBC_MGR(odbcapis_nxl_ins,.)##FUNC(__VA_ARGS__);                                                                         \
    } while (false) /**/
#else
#define NANODBC_CALL_RC(FUNC, RC, ...) RC = odbcapis_nxl_ins.FUNC(__VA_ARGS__)
#define NANODBC_CALL(FUNC, ...) odbcapis_nxl_ins.FUNC(__VA_ARGS__)
#endif
...
// Encapsulates resources needed for column binding.
        ...
        public:
            ...
            bound_column()
                    : name_(), column_(0), sqltype_(0), sqlsize_(0), scale_(0), ctype_(0), clen_(0), blob_(false),
                      cbdata_(0), pdata_(0) {
            }
...
// clang-format on
    ...
    namespace nanodbc {
        ...
        public:
            ...
            connection_impl()
                    : env_(nullptr), dbc_(nullptr), connected_(false), transactions_(0), rollback_(false) {
            }
    ...
    namespace nanodbc {
        ...
        , async_event_(nullptr)
            ...
            statement_impl(class connection &conn)
                    : stmt_(0), open_(false), conn_(), bind_len_or_null_(), wide_string_data_(), string_data_(),
                      binary_data_()
#if defined(NANODBC_DO_ASYNC_IMPL)
                                                                                                                                    , async_(false)
        , async_enabled_(false)
        ...
        public:
            ...
            result_impl(statement stmt, long rowset_size)
                    : stmt_(stmt), rowset_size_(rowset_size), row_count_(0), bound_columns_(0), bound_columns_size_(0),
                      rowset_position_(0), bound_columns_by_name_(), at_end_(false)
#if defined(NANODBC_DO_ASYNC_IMPL)
            , async_(false)
#endif
            {
        ...
        inline void result::result_impl::get_ref_impl<time>(short column, time &result) const {
            ...
            switch (col.ctype_) {
                ...
                case SQL_C_TIMESTAMP: {
                    timestamp stamp = *reinterpret_cast<timestamp *>(col.pdata_ + rowset_position_ * col.clen_);
                    time
                    t = {stamp.hour, stamp.min, stamp.sec};
                    result = t;
                    return;
                }
            }
        ...
        inline void result::result_impl::get_ref_impl(short column, T &result) const {
            ...
            switch (col.ctype_) {
                ...
                case SQL_C_TIME: {
                    ...
                    std::setlocale(LC_TIME, "");
                    char date_str[512];
                    std::strftime(date_str, sizeof(date_str), "%H:%M:%S", &st);//%T
                    std::setlocale(LC_TIME, old_lc_time);
                    convert(date_str, result);
                    return;
                }
            ...
            do {
                ...
                if (rc == SQL_SUCCESS) {
                    ...
                    while ((pair_end = std::find(beg, end, NANODBC_TEXT('\0'))) != end) {
                        ...
                        driver::attribute attr{{beg,        eq_pos},
                                               {eq_pos + 1, pair_end}};
