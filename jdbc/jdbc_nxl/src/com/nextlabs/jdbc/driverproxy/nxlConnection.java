...
public class nxlConnection implements  Connection {
    ...
    static  final String SQL_QUERY_DBINFO_POSTGRESQL = "select current_user, current_schema, current_database() , 'PostgreSQL'";
    ...
    private Properties queryJDBCSessionInfo(EMDB_DB_TYPE dbType) throws SQLException {
        ...
        else if (dbType == EMDB_DB_TYPE.DB_POSTGRESQL){
    ...
    private boolean initEnforceContext (Properties info) throws SQLException {
        ...
        {
            dbtype = EMDB_DB_TYPE.DB_SQLSERVER;
        }
        else if(driverName.toLowerCase().contains("mysql"))
        {
            dbtype = EMDB_DB_TYPE.DB_MYSQL;
        }
        else if(driverName.toLowerCase().contains("oracle"))
        {
            dbtype = EMDB_DB_TYPE.DB_ORACLE;
        }
        else if(driverName.toLowerCase().contains("hdb"))
        {
            dbtype = EMDB_DB_TYPE.DB_HANA;
        }
        else if(driverName.toLowerCase().contains("googlebigquery"))
        {
            dbtype = EMDB_DB_TYPE.DB_BIGQUERY;
        }
        else if( driverName.contains("IBM"))
        {
            dbtype = EMDB_DB_TYPE.DB_DB2;
        }
        else if (driverName.toLowerCase().contains("postgresql"))
        {
            dbtype = EMDB_DB_TYPE.DB_POSTGRESQL;
        }
        ...
        if(dbtype == EMDB_DB_TYPE.DB_SQLSERVER) {
            ...
            String ss = dbData.getURL();
            if (!ss.isEmpty())
			    serverName = ss.substring(ss.lastIndexOf("//")+2);
        }
        else if(dbtype == EMDB_DB_TYPE.DB_BIGQUERY) {
            serverName = "Google";
            try {
                String[] vec = this.url.split("ProjectId=");
                if(vec.length == 2) {
                    String str = vec[1];
                    int i = str.indexOf(";");
                    if(i > 0){
                        dataBaseName =  str.substring(0, i);
                    } else {
                        dataBaseName =  str;
                    }
                ...
                throw new SQLException("connect url is in wrong format");
            }

            try {
                String[] vec = this.url.split("OAuthServiceAcctEmail=");
                if(vec.length == 2) {
                    String str = vec[1];
                    int i = str.indexOf(";");
                    if(i > 0){
    ...
    nxlConnection(String var1, Properties var2) throws SQLException, ClassNotFoundException {
        String var3;

        try {
            String[] var4 = var1.split(";targetDriver=");
            this.url = var4[0].replace("nextlabs:", "");
            var3 = var4[1];
        ...
        this.connection = DriverManager.getConnection(this.url, var2);
