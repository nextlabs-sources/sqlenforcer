...
AstExpr *parse_boolean_expr(Lex *lex, ParseException *e) {
    ...
    if (e->_code != ParseException::RET::SUCCESS) {
        return nullptr;
    }
...
AstExpr *parse_value(Lex *lex, ParseException *e) {
    if (lex->GetCurrent()->GetType() == Token::TK_STR_LITERAL) {
        ...
        if (is_policy_regex(lex->GetCurrent()->GetWord())) {
            AstConstantValue *c = new AstConstantValue(AstExpr::EXPR_TYPE::C_PATTERN);
            /* todo parse the pattern */
            c->SetValue(lex->GetCurrent()->GetWord());
            lex->Next();
            return c;
        } else {
            AstConstantValue *c = new AstConstantValue(AstExpr::EXPR_TYPE::C_STRING);
            c->SetValue(lex->GetCurrent()->GetWord());
            lex->Next();
            return c;
        }
    } else if (lex->GetCurrent()->GetType() == Token::TK_DEC_NUMBER) {
        AstConstantValue *c = new AstConstantValue(AstExpr::EXPR_TYPE::C_NUMBER);
        c->SetValue(atoi(lex->GetCurrent()->GetWord().c_str()));
        lex->Next();
        return c;
    } else if (lex->GetCurrent()->GetType() == Token::TK_NULL) {
        AstConstantValue *c = new AstConstantValue(AstExpr::EXPR_TYPE::C_NULL);
        lex->Next();
        return c;
        ...
        lex->Next();
        AstConstantValue *r = new AstConstantValue(AstExpr::EXPR_TYPE::C_ARRAY);
        r->SetValue(array);
        return r;
    } else {
...
AstExpr *parse_predicate(Lex *lex, ParseException *e) {
    if (lex->GetCurrent()->GetType() == Token::TK_L_PAREN) {
        ...
        return new AstUnaryOpExpr(AstExpr::EXPR_TYPE::NOT, predicate);
    } else {
        ...
        if (value_right->GetExprType() == AstExpr::EXPR_TYPE::C_PATTERN ||
                value_left->GetExprType() == AstExpr::EXPR_TYPE::C_PATTERN ) {
            if (op != AstExpr::EXPR_TYPE::COMP_EQ && op != AstExpr::EXPR_TYPE::COMP_NEQ) {
                e->_code = ParseException::RET::FAIL;
                delete (value_left); delete (value_right);
                return nullptr;
            }
            op = op == AstExpr::EXPR_TYPE::COMP_EQ ? AstExpr::EXPR_TYPE::LIKE : AstExpr::EXPR_TYPE::NOT_LIKE;    /* operator will be overload */
        }
...
AstColumnRef::VAL_TYPE to_val_type(AttributeInfo::ATTR_TYPE attr_type) {
    switch (attr_type) {
        case AttributeInfo::A_NUMBER : return AstColumnRef::VAL_TYPE::CC_NUMBER;
        case AttributeInfo::A_STRING : return AstColumnRef::VAL_TYPE::CC_STRING;
        case AttributeInfo::A_MULTI : return AstColumnRef::VAL_TYPE::CC_MULTI;
        default: return AstColumnRef::VAL_TYPE::CC_OTHER;
    }
}
...
AstExpr *parse_column_ref(Lex *lex, ParseException *e) {
    ...
    if (is_type(lex->GetCurrent()->GetType(), col_type)) {
        ...
        AstColumnRef::VAL_TYPE val_type = AstColumnRef::VAL_TYPE::CC_OTHER;
        if (e->_syms == NULL) { val_type = AstColumnRef::VAL_TYPE::CC_OTHER;
        } else {
            switch (col_type) {
            case AstColumnRef::COL_TYPE::SUB: {
                    assert(ids.size() == 1);
                    val_type = to_val_type(e->_syms->GetAttrTypeByPmnameAttrName("user", ids.back()->GetId()));
                } break;
                case AstColumnRef::COL_TYPE::APP: {
                    assert(ids.size() == 1);
                    val_type = to_val_type(e->_syms->GetAttrTypeByPmnameAttrName("application", ids.back()->GetId()));
                } break;
                case AstColumnRef::COL_TYPE::HOST: {
                    assert(ids.size() == 1);
                    val_type = to_val_type(e->_syms->GetAttrTypeByPmnameAttrName("host", ids.back()->GetId()));
                } break;
                case AstColumnRef::COL_TYPE::RES: {
                    assert(ids.size() == 2);
                    val_type = to_val_type(e->_syms->GetAttrTypeByPmnameAttrName(ids[0]->GetId(), ids.back()->GetId()));
                } break;
                case AstColumnRef::COL_TYPE::ACTION: { val_type = AstColumnRef::VAL_TYPE::CC_STRING; } break;
                default: { val_type = AstColumnRef::VAL_TYPE::CC_OTHER; }
            }
        ...
        return new AstColumnRef(AstColumnRef::COL_TYPE::OTHER, AstColumnRef::VAL_TYPE::CC_OTHER, ids);
    }
}
